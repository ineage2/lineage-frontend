<script setup lang="ts">

</script>

<template>
    <section class="fixed top-0 z-20 h-dvh">
        <div class="px-5 py-4 h-full flex gap-4">
            <ul class="flex flex-col gap-7 items-center justify-center">
                <li data-aos="fade-right" data-aos-duration="200" data-aos-delay="750">
                    <ui-button-secondary to="/" icon="bi:discord"/>
                </li>
                <li data-aos="fade-right" data-aos-duration="400" data-aos-delay="750">
                    <ui-button-secondary to="/" icon="bi:telegram"/>
                </li>
                <li data-aos="fade-right" data-aos-duration="600" data-aos-delay="750">
                    <ui-button-secondary to="/" icon="bi:youtube"/>
                </li>
            </ul>
        </div>
    </section>
</template>

<style scoped>
</style>
